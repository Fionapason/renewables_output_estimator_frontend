...
// SYNC THE BUTTONS AND NUMBERS WITH THE CURRENT SELECTION
  ...
  spacingInput.value  = ref.spacing.toFixed(2);
  // compute GCR = y / spacing
  const currentGcr = ref.y / ref.spacing;
  gcrInput.value     = currentGcr.toFixed(3);

  document.getElementById('polygonOptions').style.display = 'block';
  rightSouthBtn   .classList.toggle('active', ref.orientation === 'south');
  rightDownslopeBtn.classList.toggle('active', ref.orientation === 'downslope');
}
